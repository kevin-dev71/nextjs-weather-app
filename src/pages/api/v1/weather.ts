...
import type { WeatherByCityApiResponse } from "@/src/ts/interfaces";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const {
      query: { city, units = OpenWeatherParams.METRIC, lang = "es" },
    } = req;
    // TODO: Check if is valid `units`
    const { data }: { data: WeatherByCityApiResponse } = await openWeatherAxiosInstance.get(
      `/data/2.5/weather?q=${city}&units=${units}&lang=${lang}&appid=${OPENWEATHER_API_KEY}`
    );
